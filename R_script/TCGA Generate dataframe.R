#240719 TCGA datasetを各行に遺伝子、各列にサンプル名のデータセットにする。
#データダウンロードの時に、manifest fileを使う場合はターミナルで以下のコマンドを実行する。
# gdc-clientが保存されているところへ移動
# cd "/Users/nakanotakato/Desktop/2024 Igaki lab/Data/2407/RNAseq 240719 normal vs cancer TCGA-BRCA"
# ./gdc-client download -m ./gdc_manifest.2024-07-22.txt

#次に、ダウンロードしたtsvファイルは、全てそれぞれのディレクトリが作られて保存されているので、tsvファイルをディレクトリから出す。
#ターミナルで以下を実行する。
#cd "/Users/nakanotakato/Desktop/2024 Igaki lab/Data/2407/RNAseq 240719 normal vs cancer TCGA-BRCA/"
...
directory_path <- "/Users/nakanotakato/Desktop/2024 Igaki lab/Data/2407/RNAseq 240722 TCGA_PAAD/TCGA_PAAD/"
output_path <- "/Users/nakanotakato/Desktop/2024 Igaki lab/Data/2407/RNAseq 240722 TCGA_PAAD/TCGA R output"
current_dir <- "/Users/nakanotakato/Desktop/2024 Igaki lab/Data/2407/RNAseq 240722 TCGA_PAAD/"
...
if (!dir.exists(TCGA_R_output_dir)) {
  ...
  cat("TCGA R outputディレクトリが作成されました：", TCGA_R_output_dir, "\n")
} else {
  cat("TCGA R outputディレクトリは既に存在します：", TCGA_R_output_dir, "\n")
}
...
reference_file <- file.path(directory_path, "033fbcc1-7984-4946-ba49-cd58bb641b0b.rna_seq.augmented_star_gene_counts.tsv")
